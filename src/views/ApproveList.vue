<template>
    <div class="container">
        <top-bar head-title="审批列表"></top-bar>
        <div class="main">
            <nav-bar :nav-bar-data="navBarData" @query="getApproveList">
                <div class="nav-bar-content">
                    <search-bar :search-bar-data="searchBarData" @search="fuzzySearch"></search-bar>
                    <p v-if="!approveList.length" class="noData">暂无数据</p>
                    <div class="weui_panel" v-for="item in approveList" @click="viewApproveDetail($index)">
                        <div class="weui_panel_hd">{{item.reimburseTitle}}</div>
                        <div class="weui_panel_bd">
                            <div class="weui_media_box weui_media_text">
                                <p class="weui_media_desc">提交日期：{{item.reimburseDate}}</p>
                                <p class="weui_media_desc">申请人：{{item.applicantName}}</p>
...
<script>
    ...
    var approveListUrl = urlAPI.approveList;                    // 查询报销单列表接口
    export default{
        data: function () {
            return {
                approveList: [],                // 审批列表数据
        ...
        methods: {
            getApproveList: function (keyWord) {
                // ajax请求报销列表
                this.$http.get(approveListUrl, {
                    params: {
                        'approveStatus': this.navBarData.isCurrent,   // 报销单类型
                    ...
                    before: function () {
                        ...
                        // 清空approveList里面已保存的数据
                        this.approveList = [];
                    }
                }).then(function (response) {
                    var result = response.json();
                    if (result.success) {
                        this.$set('approveList', result.approveList);
                    }
                }, function (response) {
                    // 显示错误信息
            ...
            fuzzySearch: function (keyWord) {
                this.getApproveList(keyWord);
            },
            viewApproveDetail: function (index) {
                // 跳转到报销单详情页面
                window.sessionStorage.setItem('isCurrent', JSON.stringify(this.navBarData.isCurrent));
                this.$route.router.go({name: 'approveDetail', query: {reimburseId: this.approveList[index].reimburseId}});
            }
        },
        components: {
            topBar,
